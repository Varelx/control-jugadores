...
function switchBottomView(view){
  document.getElementById('app').style.display = (view==='players') ? 'block' : 'none';
  document.getElementById('threadsPage').style.display = (view==='threads') ? 'block' : 'none';
  
  document.querySelectorAll('#bottomMenu button').forEach(b=>b.classList.remove('active'));
  const btn = Array.from(document.querySelectorAll('#bottomMenu button')).find(b=>b.textContent.includes(view==='players'?'Jugadores':'Hilos'));
  if(btn) btn.classList.add('active');
...
function loadThreads(){
  ...
  onValue(threadsRef, snap=>{
    ...
    snap.forEach(child=>{
      ...
      div.innerHTML=`
        <div class='thread-header'>${t.title} <button onclick='editThread("${id}")'>✏️</button></div>
        <div class='thread-content'>${t.content}</div>
      `;
      container.appendChild(div);

      const header = div.querySelector('.thread-header');
      ...
      get(ref(db,'users/'+auth.currentUser.uid)).then(snap=>{
        const data = snap.val();
        if(data && data.role==='admin'){
          header.querySelector('button').style.display='inline';
          document.getElementById('addThreadBtn').style.display='block';
        }
      });
    });
  });
...
document.getElementById('saveThreadBtn').addEventListener('click', ()=>{
  ...
  push(threadsRef,{title, content});
  document.getElementById('threadTitle').value='';
  document.getElementById('threadContent').value='';
  document.getElementById('addThreadForm').style.display='none';
});

// Editar hilo
window.editThread = function(id){
  const newContent = prompt('Editar contenido:');
  if(newContent !== null) set(ref(db,'threads/'+id+'/content'), newContent);
}
