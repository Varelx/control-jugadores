...
window.renderAttendanceTable=function(id,attendance){
  ...
  for(const date in attendance){ const tr=document.createElement('tr'); tr.innerHTML=`<td>${date}</td><td>${attendance[date]?'✅':'❌'}</td>`; table.appendChild(tr); }
}
...
function updateAttendanceButtons(id,attendance){
  ...
  if(attendance && attendance[today]!==undefined){ if(attendance[today]) asistBtn.classList.add('asistio'); else faltaBtn.classList.add('falto'); }
}
