...
document.getElementById('loginBtn').addEventListener('click',()=>{
  const email=document.getElementById('email').value;
  const password=document.getElementById('password').value;
  signInWithEmailAndPassword(auth,email,password).catch(e=>showMsg(e.message,'error'));
});
...
document.getElementById('adminBtn').addEventListener('click',()=>{
  const user=auth.currentUser; if(!user)return alert('No has iniciado sesión');
  get(ref(db,'users/'+user.uid)).then(snap=>{
    const data=snap.val();
...
window.approveUser=function(uid,btn){ set(ref(db,'users/'+uid+'/status'),'aprobado').then(()=>btn.parentNode.remove()); }
...
function renderPlayerCard(id,p,container){
  ...
  div.innerHTML=`
    <div class='info'>
      <input value='${p.name}' onchange='updateField("${id}","name",this.value)' style="font-weight:600;font-size:1.1em;width:100%;border:none;background:transparent;">
...
function updateAttendanceButtons(id,attendance){
  const today=new Date().toISOString().slice(0,10);
  const asistBtn=document.getElementById('asist_'+id); const faltaBtn=document.getElementById('falta_'+id);
...
function addExercise(){
  ...
  const fileInput=document.getElementById('exerciseImageInput');

  if(!name){ alert('Nombre requerido'); return; }

  const refEx=push(ref(db,'exercises'));
  set(refEx,{name,material,space,players,moreInfo,category,imageUrl:''})
    .then(()=>{
      if(fileInput.files[0]) uploadExerciseImage(refEx.key,fileInput.files[0]);
    });

  clearForm(['exerciseName','exerciseMaterial','exerciseSpace','exercisePlayers','exerciseMoreInfo','exerciseCategory']);
  document.getElementById('addExerciseForm').style.display='none';
}

async function uploadExerciseImage(id,file){
  const storageRef = sRef(storage,'exercises/'+Date.now()+'_'+file.name);
  await uploadBytes(storageRef,file);
  const url=await getDownloadURL(storageRef);
  set(ref(db,'exercises/'+id+'/imageUrl'),url);
}
...
window.renderExerciseCard=function(id,ex,container){
  ...
  div.innerHTML=`
    <div class='info'>
      <input value='${ex.name}' onchange='updateExerciseField("${id}","name",this.value)' style="font-weight:600;font-size:1.1em;width:100%;border:none;background:transparent;">
      <button onclick='toggleExerciseDetails("${id}")'>Ver / Editar</button>
    </div>
    <div class='exercise-details' id='details_${id}' style='display:none; margin-top:10px;'>
